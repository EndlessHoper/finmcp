...
type OutputOptions = z.infer<typeof outputOptionsSchema>;
    ...
    proc.on("close", (code) => {
      ...
      try {
        ...
        if (!parsed.ok) {
          ...
          (error as Error & { details?: string }).details = parsed.details;
          reject(error);
          return;
        }
...
function formatErrorMessage(toolName: string, message: string): string {
...
function isDataFrame(value: unknown): value is { __type__: "dataframe"; columns: string[]; index: unknown[]; data: unknown[][] } {
  return typeof value === "object" && value !== null && (value as { __type__?: string }).__type__ === "dataframe";
}

function isSeries(value: unknown): value is { __type__: "series"; name?: string | null; index: unknown[]; data: unknown[] } {
  return typeof value === "object" && value !== null && (value as { __type__?: string }).__type__ === "series";
}
...
function registerYfTool<TSchema extends z.ZodTypeAny>(
  ...
  (server as any).registerTool(
    ...
    async (params: z.infer<TSchema>, _extra: unknown) => {
      const { response_format, preview_limit, save, ...args } = params as z.infer<typeof schema> & OutputOptions;
      try {
        const result = await callBridge(action, args as Record<string, unknown>);
